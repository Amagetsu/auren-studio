...
function projectCard(project, index, p) { const projectLink = `href="${esc(project.href)}" target="_blank" rel="noopener noreferrer"`; const copy = { category: p[`${project.id}Category`], description: p[`${project.id}Description`] || project.description, industry: p[`${project.id}Industry`] || project.industry, benefit: p[`${project.id}Benefit`] || project.benefits, features: p[`${project.id}Features`] || project.features }; const domain = new URL(project.href).hostname; return `<article class="project-feature case-card reveal ${project.id === 'flowers' ? 'project-feature-secondary' : ''}" style="--delay:${index * 80}ms"><a class="project-preview project-preview-link" ${projectLink} aria-label="${esc(project.name)} — ${p.view}"><div class="project-window"><div class="window-top"><span>${domain}</span><span>↗</span></div>${projectPreview(project)}</div></a><div class="project-info"><div class="project-meta"><span class="project-demo-label">${p.demoBadge}</span><span>${project.id === 'flowers' ? '2026' : '2024'}</span></div><h3><a class="project-title-link" ${projectLink}>${esc(project.name)}</a></h3><p class="project-category"><strong>${p.industryLabel}:</strong> ${esc(copy.industry)}</p><p>${esc(copy.description)}</p><p class="project-benefit"><strong>${p.benefitLabel}:</strong> ${esc(copy.benefit)}</p><div class="task-list"><span>${p.tasks}</span>${copy.features.map((feature) => `<b>${icon('check')} ${esc(feature)}</b>`).join('')}</div><a class="button button-outline" ${projectLink}>${p.view} ${icon('arrow')}</a></div></article>`; }
...
function whySection() { const w = get('why'); return `<section class="section why-section"><div class="container"><div class="section-heading reveal"><div><p class="eyebrow">${w.eyebrow}</p><h2>${w.title}</h2></div></div><div class="why-grid">${w.items.map((item, i) => `<article class="why-card reveal" style="--delay:${i * 60}ms"><span class="card-index">0${i + 1}</span><h3>${item.title}</h3><p>${item.text}</p></article>`).join('')}</div></div></section>`; }
...
function advantagesSection() { const a = get('advantages'); return `<section class="section dark-section advantages-section"><div class="container advantage-layout"><div class="advantage-intro reveal"><p class="eyebrow">${a.eyebrow}</p><h2>${a.title}</h2><p>${a.text}</p></div><div class="advantage-list benefits-list">${a.items.map((item, i) => `<div class="advantage-item reveal" style="--delay:${i * 70}ms"><span class="benefit-icon" aria-hidden="true">${['✦', '↗', '◌', '⌁', '⌖', '✓'][i]}</span><div><h3>${item.title}</h3><p>${item.text}</p></div></div>`).join('')}</div></div></section>`; }

function processSection() { const p = get('process'); return `<section class="section process-section" id="process"><div class="container"><div class="section-heading reveal"><div><p class="eyebrow">${p.eyebrow}</p><h2>${p.title}</h2></div></div><div class="process-timeline process-four-steps">${p.steps.map((step, i) => `<article class="process-step reveal" style="--delay:${i * 70}ms"><div class="timeline-marker"><span>${step.n}</span></div><div class="process-step-copy"><h3>${step.title}</h3><p>${step.text}</p></div></article>`).join('')}</div></div></section>`; }

function technologySection() { const t = get('technology'); return `<section class="technology-strip"><div class="container technology-inner"><p class="eyebrow">${t.eyebrow}</p><h2>${t.title}</h2><div class="technology-list">${t.items.map((item) => `<span>${item}</span>`).join('<i aria-hidden="true">•</i>')}</div></div></section>`; }
...
function auditSection() { const a = get('audit'); return `<section class="audit-section"><div class="container audit-inner reveal"><div><p class="eyebrow">${a.eyebrow}</p><h2>${a.title}</h2><p>${a.text}</p></div><div class="audit-action"><a class="button button-primary" href="#contact-form">${a.cta} ${icon('arrow')}</a><small>${a.note}</small></div></div></section>`; }
...
function render() {
  ...
  document.querySelector('#app').innerHTML = `${header()}${hero()}${whySection()}${projectsSection()}${servicesSection()}${advantagesSection()}${processSection()}${technologySection()}${aboutSection()}${faqSection()}${auditSection()}${contactSection()}${footer()}`;
  bindInteractions();
  observeReveals();
}
