...
function projectCard(project, index, p) { const projectLink = `href="${esc(project.href)}" target="_blank" rel="noopener noreferrer"`; const copy = { category: p[`${project.id}Category`], description: p[`${project.id}Description`] || project.description }; const domain = new URL(project.href).hostname; return `<article class="project-feature case-card reveal ${project.id === 'flowers' ? 'project-feature-secondary' : ''}" style="--delay:${index * 80}ms"><a class="project-preview project-preview-link" ${projectLink} aria-label="${esc(project.name)} — ${p.view}"><div class="project-window"><div class="window-top"><span>${domain}</span><span>↗</span></div>${projectPreview(project)}</div></a><div class="project-info"><div class="project-meta"><span class="project-demo-label">Demo Project</span><span>${project.id === 'flowers' ? '2026' : '2024'}</span></div><h3><a class="project-title-link" ${projectLink}>${esc(project.name)}</a></h3><p class="project-category"><strong>Branche:</strong> ${esc(project.industry)}</p><p>${esc(copy.description)}</p><p class="project-benefit"><strong>Mehrwert:</strong> ${esc(project.benefits)}</p><div class="task-list"><span>${p.tasks}</span>${project.features.map((feature) => `<b>${icon('check')} ${esc(feature)}</b>`).join('')}</div><a class="button button-outline" ${projectLink}>${p.view} ${icon('arrow')}</a></div></article>`; }
